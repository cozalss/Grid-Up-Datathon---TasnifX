...
CIK["1_gurultu_tabani"] = {
    ...
    "ZAYIFLATMA_YENIDEN": {
        "aciklama": ("p12e lambda'yi TOHUM gurultusuyle hesapladi (7.60e-06). "
        ...
        "yorum": ("lambda<=0: gozlenen x sacilimi (var 1.09e-04) OLCULEN vekil hata "
...
def sim(n_tekrar, sigma_vekil, sd_lb, b_true, kural_gercek=False, esik_true=0.02,
        b_kucuk_true=0.0, rng=None):
    ...
    for _ in range(n_tekrar):
        ...
        res.append((k["buyuk"]["n"], k["buyuk"]["b"], k["buyuk"]["R2"],
                    k["kucuk"]["n"], k["kucuk"]["b"], k["kucuk"]["R2"],
                    float(bb_), float(r2b)))
    return np.array(res, dtype=float)


def ozetle(r):
    ad = ["n_buyuk", "b_buyuk", "R2_buyuk", "n_kucuk", "b_kucuk", "R2_kucuk",
          "b_brut", "R2_brut"]
    o = {}
    for i, a in enumerate(ad):
        v = r[:, i]
        v = v[np.isfinite(v)]
        o[a] = {"medyan": float(np.median(v)),
    ...
    o["P_hepsi_birden"] = float(np.mean((R2b[ok] >= 0.9) & (R2k[ok] <= 0.20) &
                                        (bk[ok] < 0) & (bb[ok] >= 1.8)))
    return o
...
for ad, anah in [("oruntu(R2b>0.9 & R2k<0.2)", "P_oruntu_R2buyuk>0.9_ve_R2kucuk<0.2"),
                 ("dort_sayi_birden", "P_hepsi_birden"),
                 ("R2k<=0.036", "P_R2kucuk<=0.036")]:
    LR[ad] = {"P_H0_kural_YOK": h0[anah], "P_H1_kural_GERCEK": h1[anah],
...
LR["SONUC"] = ("Adil karsilastirmada LR(kural gercek / kural yok) = %.2f. "
               "27 dosyalik veri iki hipotezi AYIRT EDEMIYOR."
               % (h1["P_hepsi_birden"] / h0["P_hepsi_birden"]))
...
CIK["2_simulasyon"] = {
    "n_tekrar": N,
    "gozlenen": GOZ,
    "kurulum": ("gercek dCV_i = dLB_i / b_true (gozlenen 27 dosyanin dLB dagilimi "
...
ESLES_B = {"v27": ("v27 harman 3/1/1", "v18 (dun)"),
           "v30": ("v30 kurgusu", "v30 (v27+buzme)"),
...
def cift_analiz(esles):
    ...
    for i in range(len(ad)):
        for j in range(i + 1, len(ad)):
            ...
            ciftler.append({"cift": a + "->" + b, "gercek_dCV": dcv, "gercek_dLB": dlb,
                            "oran": dlb / dcv, "band_ici_mi": abs(dcv) < 0.02})
    ...
    return {"ciftler": ciftler,
            ...
            "isaret_uyumu": bool(all(np.sign(c["gercek_dCV"]) == np.sign(c["gercek_dLB"])
                                     for c in ciftler))}
...
CIK["3_gercek_dCV_kontrolu"] = {
    ...
    "eslestirme_notu": ("rekor.jsonl'de v30/v33/v36/v39/v46 blok skorlari 'SON ISLEM "
...
for sd_lb in (0.0016, 0.002, 0.005, 0.010):
    ...
    sinama["sd_LB=%.4f" % sd_lb] = {"se_b": se_b, "z": b_ko2 / se_b,
                                    "tek_yonlu_p_b0icin": float(0.5 * math.erfc(
                                        (b_ko2 / se_b) / math.sqrt(2)))}
CIK["3_gercek_dCV_kontrolu"]["3b_b0_hipotezi_sinamasi"] = {
    "b_kesmesiz_2_bagimsiz_cift": b_ko2,
    "artiklarin_RMS_si": sd_artik,
    "aciklama": ("Kural gercekse bu ciftlerde (hepsi |dCV|<0.02) b=0 olmali ve dLB "
                 "sirf CV->LB sapma gurultusu olmali. sd_LB varsayimina duyarlilik:"),
    "duyarlilik": sinama,
    "isaret_sinamasi": {"n_bagimsiz": 2, "isaret_uyumu": "2/2",
                        "P_sirf_sansla": 0.25},
...
for ad, o in [("oran_0.0", 0.0), ("oran_0.25", 0.25), ("oran_0.5", 0.5),
              ("oran_0.568_gercek_cift_A", 0.5680), ("oran_0.967_gercek_cift_B", 0.967),
              ("oran_1.0", 1.0), ("oran_1.428_brut_vekil", B_BRUT)]:
...
# ---- ekrana ozet
print("BAND SINIRI 0.02 = %.2f x sigma_dosya(%.5f) = %.2f x sigma_cift(%.5f)"
...
for k, v in senaryolar.items():
    print("%-52s R2b=%.3f R2k=%.3f bk=%+.2f bb=%+.2f  P(oruntu)=%.3f P(hepsi)=%.3f"
          % (k, v["R2_buyuk"]["medyan"], v["R2_kucuk"]["medyan"],
             v["b_kucuk"]["medyan"], v["b_buyuk"]["medyan"],
             v["P_oruntu_R2buyuk>0.9_ve_R2kucuk<0.2"], v["P_hepsi_birden"]))
print()
for ad in ("eslestirme_A_p12e_ile_ayni", "eslestirme_B_v46blok_ESKI_sonislem_yani_v47LB"):
    a = CIK["3_gercek_dCV_kontrolu"][ad]
    print(ad, "b_kesmesiz=%.3f (2 bagimsiz: %.3f) oranlar=%s hepsi band ici=%s"
          % (a["kesmesiz_egim_3cift"], a["kesmesiz_egim_2_BAGIMSIZ_cift"],
             ["%.3f" % o for o in a["oranlar"]], a["hepsi_band_icinde_mi"]))
print()
