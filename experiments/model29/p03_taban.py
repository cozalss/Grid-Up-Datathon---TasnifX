...
tab = df.groupby("kova", observed=False).agg(
    n=("e2", "size"),
    pay_satir=("e2", lambda s: len(s) / len(df)),
    e2_ort=("e2", "mean"),
...
sg = df.groupby("soguk").agg(
    n=("e2", "size"), e2_ort=("e2", "mean"), pay_hata=("e2", lambda s: s.sum() / e2.sum())
)
sg["rmsle"] = np.sqrt(sg.e2_ort)
sg["sapma"] = df.groupby("soguk").apply(lambda d: float((d.p - d.yd).mean()), include_groups=False)
print("\n--- soguk/sicak ---")
print(sg.to_string())
...
with open(
    os.path.join(os.path.dirname(os.path.abspath(__file__)), "p03_taban.json"),
    "w",
    encoding="utf-8",
) as f:
