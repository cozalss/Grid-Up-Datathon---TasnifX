...
PAR = dict(objective="regression", metric="l2", learning_rate=0.04,
           num_leaves=255, min_data_in_leaf=100, feature_fraction=0.6,
...
for et, oz, msk in (("sicak", TUM, 0), ("soguk", SOZ, 1)):
    ...
    m = lgb.train(PAR, dA, num_boost_round=4000, valid_sets=[dB],
                  callbacks=[lgb.early_stopping(150, verbose=False)])
    ni = max(50, int(m.best_iteration * 1.1))
    log(et, "n", len(a), "best_iter", m.best_iteration, "-> nihai", ni,
        "ayrilan RMSE", round(float(m.best_score["valid_0"]["l2"]) ** 0.5, 5))
    mf = lgb.train(PAR, lgb.Dataset(a[oz], label=a.y - a._ofs,
                                    categorical_feature=KAT), num_boost_round=ni)
    for ad in ("yaz25", "test"):
        d = bloklar[ad]
        s = d.soguk == msk
        if s.sum() == 0:
            continue
        d.loc[s, "p2"] = np.clip(mf.predict(d.loc[s, oz], num_iteration=ni)
