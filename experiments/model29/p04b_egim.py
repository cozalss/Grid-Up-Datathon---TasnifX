...
for ad, m in (
    ("hedef SIFIR", sifir),
    ("soguk trafo", Y.soguk_mu.values == 1),
    ("|artik|>1", buyuk),
    ("sifir & |artik|>1", sifir & buyuk),
):
    print(
        f"  {ad:20s} satir%={m.mean():.4f}  SSE%={(Y.w[m] * Y.r[m] ** 2).sum() / tot:.4f}",
        flush=True,
    )
    R[f"ayrisma_{ad}"] = {
...
ki_d, ki_y = (
    DIS.ilce_key.isin(KIYI).astype(float).values,
    Y.ilce_key.isin(KIYI).astype(float).values,
)
su_d, su_y = (
    DIS.ilce_key.isin(SULAMA).astype(float).values,
    Y.ilce_key.isin(SULAMA).astype(float).values,
)
