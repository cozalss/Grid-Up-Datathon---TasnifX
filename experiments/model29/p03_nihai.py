...
poz = ye > 0
z = (ye == 0).astype(int)
P = dict(metric="l2", learning_rate=0.04, num_leaves=63, min_data_in_leaf=200,
...
for ad, (Xtr_, Xte_, iki, ek) in {
    ...
    for s in (7, 17, 27):
        if iki:
            p0 = lgb.train(dict(PC, seed=s), lgb.Dataset(Xtr_, z), num_boost_round=TUR).predict(Xte_)
            pp = lgb.train(dict(P, seed=s, **ek), lgb.Dataset(Xtr_[poz], ye[poz]), num_boost_round=TUR).predict(Xte_)
            p = (1 - p0) * pp
        else:
            p = lgb.train(dict(P, seed=s, **ek), lgb.Dataset(Xtr_, ye), num_boost_round=TUR).predict(Xte_)
        per.append(olc(p))
        log(f"{ad} tohum {s}: {per[-1]['rmsle']:.5f} (agirlikli {per[-1]['test_agirlikli']:.5f})")
    R["kosular"][ad] = {"tohumlar": per,
...
for ad in ("B_iki_huber", "C_iki_huber_kom"):
    ...
    R["kosular"][ad]["kazanc_vs_A"] = {
        "yaz25": a["rmsle"] - o["rmsle"], "test_agirlikli": a["test_agirlikli"] - o["test_agirlikli"]}
json.dump(R, open(os.path.join(B, "p03_nihai.json"), "w", encoding="utf-8"), indent=1, ensure_ascii=False)
