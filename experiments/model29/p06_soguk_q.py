...
def main():
    ...
    print(
        f"soguk egitim {len(ds):,} satir / {ds.tanim.nunique():,} trafo   "
    ...
    q = np.mean(
        [
            lgb.train(dict(PK, seed=s), lgb.Dataset(Xd, zd), num_boost_round=TUR).predict(Xy)
            for s in (7, 17, 27)
        ],
        axis=0,
    )
    R = dict(
        ...
        ozellik=len(KOL),
        agac=TUR,
        ic_secim={str(k): round(v, 4) for k, v in ic.items()},
        auc_yaz25_soguk=round(auc(zy, q), 4),
        q_ort=round(float(q.mean()), 4),
        gercek_oran=round(float(zy.mean()), 4),
    ...
    qd = np.mean(
        [
            lgb.train(dict(PK, seed=s), lgb.Dataset(Xd[gm], zd[gm]), num_boost_round=TUR).predict(
                Xd[~gm]
            )
            for s in (7, 17, 27)
        ],
