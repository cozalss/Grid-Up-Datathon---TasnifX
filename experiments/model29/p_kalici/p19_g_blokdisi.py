...
BURA = os.path.dirname(os.path.abspath(__file__)); sys.path.insert(0, BURA)
...
q = test_dagilimi(T[T.soguk_mu == 1]); del T
E = egitim()
MW, KZ, TS = {}, {}, {}
for b in BLOKLAR:
    blk = E[E._blok == b]; sog = blk[blk.soguk_mu == 1].reset_index(drop=True)
    y = np.log1p(sog.tuketim.to_numpy(dtype="float64").clip(0))
    w = agirlik(sog, q); m = kova(sog)[0] == PG_HAKIM; sic = sicak_rmsle(b)
    z = np.load(os.path.join(DN, f"soguk_tahmin_{b}.npz"))
    ts = [t for t in sorted({int(k.split("_")[0]) for k in z.files})
    ...
    Y = {"B_std": W[0]*cat7 + W[1]*xgb + W[2]*lgb,
    ...
    for k, v in Y.items():
        ...
        KZ[b][k] = dict(agr=round(wrmsle(rB, w) - wrmsle(r, w), 5),
...
for h in BLOKLAR:
    ...
    R["secim"][h] = dict(secilen=s, puan=puan,
                         hedefte=(None if s == "B_std" else KZ[h][s]))
...
R["durust_blokdisi_bilesim"] = dict(bloklar=b, ort=round(float(np.mean(b)), 5),
                                    pozitif=int(sum(1 for x in b if x > 0)), n=len(b))
print("\nDURUST BLOK-DISI:", R["durust_blokdisi_bilesim"])
json.dump(R, open(os.path.join(BURA, "p19_g_blokdisi.json"), "w", encoding="utf-8"),
