...
def kur(ad):
    """zy'deki ad bicimini ('kolon', 'kolon:x_sv', 'kolon:ust10') coz."""
    if ":" in ad:
        kol, kip = ad.split(":", 1)
    else:
        kol, kip = ad, ""
    ...
    if kip == "x_sv":
        a, b = st(st(xt) * svT), st(st(xb) * svB)
    elif kip == "x_soguk":
        a, b = st(st(xt) * sgT), st(st(xb) * sgm)
    elif kip == "ust10":
        fv = xt[np.isfinite(xt)]
        if fv.size == 0:
            return None, None
        q = np.quantile(fv, 0.9)
        a, b = st((xt > q).astype(np.float64)), st((xb > q).astype(np.float64))
    else:
        a, b = st(xt), st(xb)
    return a, b


with open(os.path.join(BURA, "m118_tarama.json")) as fh:
    TARAMA = json.load(fh)
