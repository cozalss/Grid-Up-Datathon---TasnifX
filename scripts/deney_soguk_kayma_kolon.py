...
def main() -> int:
    ...
    for ad in list(ADAYLAR)[1:]:
        ...
        sh = float(f.std(ddof=1) / np.sqrt(3))
        ...
        print(
            ...
            f"  t {td:+6.2f}  {(f > 0).sum()}/3  genele {-f.mean() * SOGUK_KATSAYI:+.5f}  {hukum}"
        )
    ...
    print("-" * 104)
    du = sum((g - tahmin[("taban", t)]) ** 2 for t in TOHUMLAR) / 3
    for ad in list(ADAYLAR)[1:]:
        ds = sum((g - tahmin[(ad, t)]) ** 2 for t in TOHUMLAR) / 3
        ...
        for K in (0, 1, 5, 10, 25, 50):
            ...
            print(
                ...
                f" {(f > 0).sum()}/3   {-f.mean() * SOGUK_KATSAYI:>+9.5f}"
            )
