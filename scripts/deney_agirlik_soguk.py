"""HARMAN AGIRLIGI -- SOGUK rejim, uc tohumla.
...
SICAK_MASKE = 1.00  # soguk uzmani
TOHUMLAR = (1000, 1001, 1002)
CAT_USTYAZIM: dict[str, object] = {"depth": 7}  # soguk uzmani
AILELER = ("cat", "xgb", "lgbm")

#: Taranacak (cat, xgb, lgbm) agirliklari. Uretim 3/1/1.
AGIRLIKLAR: tuple[tuple[float, float, float], ...] = (
    (1, 0, 0), (0, 1, 0), (0, 0, 1),
    (0, 1, 1), (1, 1, 1), (1, 2, 2), (1, 3, 3), (2, 3, 3),
    (1, 2, 1), (1, 1, 2), (2, 2, 1), (2, 1, 2), (2, 2, 2),
    (2, 1, 1), (3, 2, 1), (3, 1, 2), (3, 3, 1), (3, 1, 1), (4, 1, 1),
)

KAYIT = KOK / "experiments" / "agirlik_soguk.jsonl"
...
def main() -> int:
    ...
    kolonlar += ["tk_haftanin_gunu", "tk_hafta_sonu"]  # v20 soguk yapilandirmasi
    ...
    for kol_ad, kaynak, ayikla in (("ANA (soguk ek koken almaz)", egitim, False),):
        for b in tm.BLOKLAR:
            ...
            sic = (dogrulama["soguk_mu"] == 1).to_numpy()  # SOGUK satirlar
            ...
            for tohum in TOHUMLAR:
                ...
                for aile in AILELER:
                    ...
                    onbellek[(kol_ad, b.ad, aile, tohum)] = di.egit_tahmin(
                        aile, maskeli, dogrulama, kolonlar, tohum, **ust
                    )
                    print(f"    {b.ad:6} tohum {tohum} {aile:5} bitti ({time.time() - t0:.0f} sn)")
    ...
    def skor(kol_ad: str, w: tuple[float, float, float]) -> float:
        ...
        for b in tm.BLOKLAR:
            ...
            log_t = np.mean([
                sum(wi * onbellek[(kol_ad, b.ad, a, tohum)]
                    for a, wi in zip(AILELER, w, strict=True)) / toplam
                for tohum in TOHUMLAR
            ], axis=0)
    ...
    for i, aile in enumerate(AILELER):
        ...
        a = e = skor("ANA (soguk ek koken almaz)", w)  # type: ignore[arg-type]
    ...
    en_iyi = {}
    for w in AGIRLIKLAR:
        a = e = skor("ANA (soguk ek koken almaz)", w)
        isaret = "  <- URETIM" if w == (3, 1, 1) else ""
        print(f"  {str(w):14}{a:>10.5f}{e:>13.5f}{isaret}")
        for kol_ad, v in (("ANA (soguk ek koken almaz)", e),):
    ...
    print("  Uretim agirligini ancak fark tohum gurultusunun USTUNDEYSE degistir.")
