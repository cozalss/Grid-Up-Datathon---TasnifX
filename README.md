...
| **29 Ağustos planı** | **HAK 1**: `submissions/tuketim_m3_hl1_capali.csv` — yeni model (ileri-pencere doğrudan tahmin, Huber+L1). `Q = 0,122746`, **başabaş `1,06482`**<br>**HAK 2**: `python experiments/model29/m50_harman_coz.py tuketim_m3_hl1_capali.csv <HAK1 skoru>` → optimum `κ*` harmanı<br>`S=0,99 → 0,98130` (1. sıra) · `S=1,00553 → 0,99015` (2. sıra) · `S=1,02 → 0,99665` (2. sıra)<br>Ayrıntı: [`docs/53`](docs/53-yeni-model-ve-29-agustos-plani-2026-08-28.md). `docs/52` §21'in P41/P42 planı **iptal** — o plan iki ölçülmemiş yönde `\|δ\|≥0,0497` gerektiriyordu |
...
| **Boru Hattı & Modeller** | **YENİ (2026-08-28):** `experiments/model29/m46_nihai.py` — ileri-pencere doğrudan tahmin (LightGBM Huber+L1, 3'er tohum, `v102`'nin LB-kalibre seviyesine rejim çapası). `kis26` penceresinde üretim hattının en iyi CV yığınını **%5,8 dövüyor** (1,1063 → 1,0416). Eski hat: `scripts/sota_tuketim_pipeline.py` (CatBoost+LightGBM+XGBoost) |
