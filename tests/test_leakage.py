...
from gridup.features.temporal import add_lag_features, add_rolling_features
...
class TestPurgedSplit:
    ...
    def test_train_always_precedes_validation(self, time_series_frame):
        times = time_series_frame["tarih"]
        for train_idx, valid_idx in purged_time_series_split(times, n_splits=3):
            assert times.iloc[train_idx].max() < times.iloc[valid_idx].min()

    def test_empty_series_raises(self):
        with pytest.raises(ValueError):
            list(purged_time_series_split(pd.Series([], dtype="datetime64[ns]")))
